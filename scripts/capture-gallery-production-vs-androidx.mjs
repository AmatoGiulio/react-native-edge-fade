...
const OUT = path.join(
  ROOT,
  'benchmark-results',
  'production-vs-androidx-visual'
);
const RENDERERS = ['public', 'androidx'];
...
function verify(renderer) {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    if (renderer === 'public') {
      ...
      const expectedUi =
        'gallery-renderer requested=androidx active=androidx';
      const expectedLog =
        'Renderer active: requested=androidx active=androidx';
      if (lastUi.includes(expectedUi) || lastLogs.includes(expectedLog)) return;

      if (
        lastUi.includes('gallery-renderer requested=androidx active=off') ||
        lastLogs.includes('Renderer active: requested=androidx active=off')
      ) {
        throw new Error(
          `AndroidX official backend is disabled. Rebuild with the AndroidX blur opt-in.\n${lastLogs}`
        );
      }
    }
...
console.log(
  `Production fidelity capture / radius=${radiusPx}px / edges=${edges} / curve=${curve}`
);
