...
const cards = profiles.map((profile, index) => {
  ...
  return [
    ...
    '<header><strong>' +
      ...
      '</span></header>',
    '<div class="pair">',
    '<figure><figcaption>closed · normalized focus</figcaption><img src="./' + prefix + '-closed-focus.svg?t=' + Date.now() + '"></figure>',
    '<figure><figcaption>open · normalized focus</figcaption><img src="./' + prefix + '-open-focus.svg?t=' + Date.now() + '"></figure>',
    '</div>',
    '</section>',
  ].join('\n');
}).join('\n');
...
const html = [
  ...
  'img{display:block;width:100%;aspect-ratio:530/565;max-height:82vh;object-fit:cover;object-position:center;background:#171717;border:1px solid #292929}',
  '</style></head><body>',
