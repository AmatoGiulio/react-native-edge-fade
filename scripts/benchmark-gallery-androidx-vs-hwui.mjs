...
const OUT_DIR = path.join(ROOT, 'benchmark-results', 'gallery-androidx-vs-hwui');
const RENDERERS = ['androidx', 'hwui-scaled'];
...
console.log(`Gallery AndroidX vs HWUI-scaled / ${model} / API ${sdk}`);
...
for (let sample = 0; sample < options.samples; sample++) {
  ...
  for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
    ...
    const order = sample % 2 === 0
      ? ['androidx', 'hwui-scaled']
      : ['hwui-scaled', 'androidx'];
    const block = [];
...
for (let sample = 1; sample <= options.samples; sample++) {
  ...
  if (!androidxRow || !hwuiRow) continue;
  ...
  pairwise.push({
    ...
    deltaHwuiVsAndroidx: {
      p95Ms:
        Number.isFinite(hwuiRow.p95Ms) && Number.isFinite(androidxRow.p95Ms)
          ? hwuiRow.p95Ms - androidxRow.p95Ms
          : null,
      p99Ms:
        Number.isFinite(hwuiRow.p99Ms) && Number.isFinite(androidxRow.p99Ms)
          ? hwuiRow.p99Ms - androidxRow.p99Ms
          : null,
      jankPctPoints:
        Number.isFinite(hwuiRow.jankyPercent) && Number.isFinite(androidxRow.jankyPercent)
          ? hwuiRow.jankyPercent - androidxRow.jankyPercent
          : null,
      deadline:
        Number.isFinite(hwuiRow.deadlineMissed) && Number.isFinite(androidxRow.deadlineMissed)
          ? hwuiRow.deadlineMissed - androidxRow.deadlineMissed
          : null,
    },
  });
}
...
const pairwiseMedianDelta = {
  p95Ms: median(pairwise.map((pair) => pair.deltaHwuiVsAndroidx.p95Ms)),
  p99Ms: median(pairwise.map((pair) => pair.deltaHwuiVsAndroidx.p99Ms)),
  jankPctPoints: median(pairwise.map((pair) => pair.deltaHwuiVsAndroidx.jankPctPoints)),
  deadline: median(pairwise.map((pair) => pair.deltaHwuiVsAndroidx.deadline)),
};
...
const deltaHwuiVsAndroidx = {
  jankDeltaPctPoints:
    androidx.weightedJankPercent == null || hwui.weightedJankPercent == null
      ? null
      : hwui.weightedJankPercent - androidx.weightedJankPercent,
  p95DeltaMs:
    androidx.medianP95Ms == null || hwui.medianP95Ms == null
      ? null
      : hwui.medianP95Ms - androidx.medianP95Ms,
  p99DeltaMs:
    androidx.medianP99Ms == null || hwui.medianP99Ms == null
      ? null
      : hwui.medianP99Ms - androidx.medianP99Ms,
  deadlineDelta: hwui.deadlineMissed - androidx.deadlineMissed,
};
...
console.log('\n=== Pairwise deltas (HWUI - AndroidX) ===');
for (const pair of pairwise) {
  console.log(
    `sample=${pair.sample} refresh=${pair.refreshHz ?? 'n/a'}Hz ` +
      `Δp95=${pair.deltaHwuiVsAndroidx.p95Ms ?? 'n/a'}ms ` +
      `Δp99=${pair.deltaHwuiVsAndroidx.p99Ms ?? 'n/a'}ms ` +
      `Δjank=${pair.deltaHwuiVsAndroidx.jankPctPoints?.toFixed(2) ?? 'n/a'}pp ` +
      `Δdeadline=${pair.deltaHwuiVsAndroidx.deadline ?? 'n/a'}`
  );
}
console.log(
...
console.log(
  `HWUI vs AndroidX: Δp95=${deltaHwuiVsAndroidx.p95DeltaMs ?? 'n/a'}ms ` +
    `Δp99=${deltaHwuiVsAndroidx.p99DeltaMs ?? 'n/a'}ms ` +
    `Δjank=${deltaHwuiVsAndroidx.jankDeltaPctPoints?.toFixed(2) ?? 'n/a'}pp ` +
    `Δdeadline=${deltaHwuiVsAndroidx.deadlineDelta}`
);
