...
const DEFAULT_OUT_DIR = path.join(ROOT, 'benchmark-results', 'gallery-public-perfetto');
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === '--image-renderer') options.imageRenderer = take();
...
function rendererLogs() {
  return adb(
    ['logcat', '-d', '-s', 'EdgeFadeProgressive:I', '*:S'],
    { trim: false }
  );
}

function verifyPublicActivation() {
  ...
  if (!activated) {
    throw new Error(`Public progressive activation was not observed.\n${logs.trim()}`);
  }
  if (/Progressive unavailable|draw failed|frame unavailable|fallback/i.test(logs)) {
    throw new Error(`Progressive fallback/failure was observed.\n${logs.trim()}`);
  }
}
...
function launch(effectEnabled) {
  ...
  const uri =
    ...
    `&radiusPx=${options.radiusPx}&cycleMs=${options.cycleMs}&image=${options.imageRenderer}`;
  const result = shell(
  ...
  if (effectEnabled) verifyPublicActivation();
}
...
function capture(effectEnabled, device) {
  const label = effectEnabled ? 'public' : 'off';
  console.log(`\n=== Public Gallery Perfetto: ${label} ===`);
  ...
  const remoteTrace = `/data/misc/perfetto-traces/edgefade-public-${label}-${id}.perfetto-trace`;
  const base = `${timestamp()}-${options.imageRenderer}-${label}-${options.radiusPx}px`;
  ...
  try {
    ...
    const metadata = {
      branch: 'codex/compose-official-sdk58',
      renderer: label,
      ...
      activationLog: effectEnabled ? rendererLogs().trim() : null,
    };
...
if (!Number.isFinite(device.sdk) || device.sdk < 31) {
  throw new Error(`Progressive Perfetto capture requires API 31+; device is API ${device.sdk}.`);
}

console.log(`Public Gallery Perfetto / ${device.model} / API ${device.sdk}`);
console.log(`radius=${options.radiusPx}px / image=${options.imageRenderer} / duration=${options.durationMs}ms`);
...
console.log('\nCapture complete. Inspect these slices in the PUBLIC trace:');
...
console.log('  EdgeFade.progressive.recordStrip.top');
console.log('  EdgeFade.progressive.recordStrip.bottom');
console.log('  EdgeFade.progressive.drawStrip.top');
console.log('  EdgeFade.progressive.drawStrip.bottom');
