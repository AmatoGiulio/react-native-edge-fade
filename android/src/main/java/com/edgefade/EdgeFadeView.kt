...
class EdgeFadeView(context: Context) : FrameLayout(context) {
  ...
  internal var progressiveReflectionGain: Float = 1f
  ...
  internal var progressiveBodyDiffusion: Float = 1f
  internal var progressiveBodyDiffusionRadius: Float = 72f
  ...
  internal fun effectiveProgressiveBackend(): String = tunerBackendOverride ?: progressiveBackend
  ...
  internal fun effectiveMaterialStrength(): Float = tunerMaterialStrengthOverride ?: progressiveMaterialStrength
  internal fun effectiveMaterialExposure(): Float = tunerMaterialExposureOverride ?: progressiveMaterialExposure
  internal fun effectiveMaterialSurface(): Float = tunerMaterialSurfaceOverride ?: progressiveMaterialSurface
  ...
  internal fun clearNativeTunerOverrides() {
    ...
    progressiveBodyDiffusion = 1f
    progressiveBodyDiffusionRadius = 72f
    tunerShowBounds = false
    nativeTuneChanged()
  }
  ...
  private fun drawNativeTunerBounds(canvas: Canvas) {
    ...
    if (fadeTop > 0f) canvas.drawRect(inset, inset, w - inset, fadeTop.coerceAtMost(h - inset), tunerBoundsPaint)
    if (fadeBottom > 0f) canvas.drawRect(inset, (h - fadeBottom).coerceAtLeast(inset), w - inset, h - inset, tunerBoundsPaint)
    if (fadeLeft > 0f) canvas.drawRect(inset, inset, fadeLeft.coerceAtMost(w - inset), h - inset, tunerBoundsPaint)
    if (fadeRight > 0f) canvas.drawRect((w - fadeRight).coerceAtLeast(inset), inset, w - inset, h - inset, tunerBoundsPaint)
  }
